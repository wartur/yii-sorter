 ...
 */
class SorterActiveRecordBehaviorFuncOntherflyEndTest extends CDbTestCase {

	/**
	 * @var string model name
	 */
	private static $className = 'SortestOntheflyEndStd';
	...
	public function testOntheflyNormalisation() {
		$srcBefore = Yii::app()->db->createCommand('SELECT id FROM sortest WHERE sort <= 1075544064 ORDER BY sort ASC')->queryColumn();
		$srcAfter = Yii::app()->db->createCommand('SELECT id FROM sortest WHERE sort > 1075544064 ORDER BY sort ASC')->queryColumn();

		$model = $this->createModel();
		$model->name = 'insert';
		$model->sorterMoveToModelAfter(119, true);
		...
		$this->assertEquals(1075544064, $this->loadModel(119)->owner->sort); // is end side normalisation sort not changd
		$this->assertNotEquals(1075544065, $this->loadModel(128)->owner->sort); // sort must changed after normalisation
	}
